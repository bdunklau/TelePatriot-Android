...
exports.checkLegal = functions.database.ref('cb_api_events/all-events/{key}').onCreate((snap2, context) => {
    if(snap2.val().event_type != 'check legal')
        return false

    var uid = snap2.val().uid
    ...
    return db.ref('administration/configuration').once('value').then(snapshot => {
        ...
        if(configuration.on_user_created == "checkVolunteerStatus") {
            ...
            var f = function(valid_actual) {
                ...
                db.ref('cb_api_events/all-events').push().set({uid: snap2.val().uid, name: snap2.val().name, email: snap2.val().email,
                ...
                db.ref('cb_api_events/check-legal-responses/'+snap2.val().uid)
                    .push()
                    .set({uid: snap2.val().uid, name: snap2.val().name, email: snap2.val().email, valid: valid})
            }
            ...
            exports.checkVolunteerStatus(snap2.val().email, allowed, notAllowed)
        }
        else {
